    ...
    chanel.bind("user-connected", ({ user }: { user: User }) => {
      ...
      if (!connectedUsersRef.current.find((u) => u.id === user.id)) {
        ...
        if (currentUserRef.current && user.id !== currentUserRef.current.id) {
          ...
          currentCard &&
            postCardChoosen(currentCard, currentUserRef.current, idRoom);
        }
      }
    });
      ...
      <div className="w-full h-full flex flex-col justify-between pb-4">
        <div className="grid grid-cols-[1fr,4fr,1fr] pt-2">
          <div
            ...
            onClick={() => router.push("/")}
          >
            <span className="transition-all hover:scale-110">🦄</span>
          </div>
          <div className="">
            <h1>{routeName}</h1>
            ...
            <div className="grid grid-cols-[1fr,3fr,1fr] grid-rows-[2fr,3fr,2fr] w-2/3 self-center py-10 gap-4">
              <div className="grid grid-rows-3 bg-light-secondary w-full h-full m-auto items-center justify-center rounded-xl col-span-1 col-start-2 row-span-1 row-start-2">
                <button
                  ...
                  disabled={cards.length !== connectedUsers.length}
